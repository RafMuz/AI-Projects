...
Path = "C:/Users/Raf/Downloads/Python/Python Project File's 2020/December 2020/Binary Neural Network ( Diabetes )/prima-indians-diabetes.csv"
...
Inputs = Data_Values [:, :8]
Expected_Outputs = Data_Values [:, 8]

# Normalizing the Inputs.
Normalized_Inputs = MinMaxScaler (feature_range = (-1, 1)).fit_transform (Inputs)

# Spliting the data into training and testing sets.
Training_Inputs, Validation_Inputs, Training_Outputs, Validation_Outputs = Train_Test_Split (Normalized_Inputs, Expected_Outputs, test_size = 0.2, random_state = 42)
...
model.add (Dense (24, input_dim = Training_Inputs.shape [1], activation = 'sigmoid', kernel_initializer = 'he_uniform'))
model.add(Dropout(0.2))
model.add (Dense (12, activation = 'sigmoid', kernel_initializer = 'he_uniform'))
#model.add(Dropout(0.2))
...
op = SGD (lr = 0.1)
model.compile (loss = 'binary_crossentropy', optimizer = op, metrics = ['accuracy'])  # Optimizer.
...
a = 100 - ((Error/ N) * 100)
print("The Accuracy is: {0}%".format (round (a, 2)))
